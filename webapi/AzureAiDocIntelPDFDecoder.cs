...
public sealed class AzureAIDocIntelPdfDecoder : IContentDecoder
{
    ...
    public async Task<FileContent> DecodeAsync(Stream data, CancellationToken cancellationToken = default)
    {
        ...
        var operation = await this._recognizerClient.AnalyzeDocumentAsync(WaitUntil.Completed, "prebuilt-read", memory, cancellationToken: cancellationToken).ConfigureAwait(false);
